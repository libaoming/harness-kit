---
name: {{PROJECT}}-ops
description: {{PROJECT}} 项目的脏活隔离子 agent（L4 上下文隔离层）。专吃重 context 的只读核查：远程/生产状态、长日志、大文档检索、数据/transcript 分析。在独立 context 跑完只回结论，让主 agent context 保持干净。当需要核查 {{PROJECT}} 线上状态、读大日志/大文档、做诊断时使用。
