...
## 多 agent 的两种拓扑：星型汇聚 vs 共享白板

L4 把「吃 context 的脏活」外包给子 agent、主线只收结论——它隐含了一个选择：**这些 agent 之间不互相通信，只把结论单向汇聚回主线**。这就是「星型」拓扑。把它讲清楚，是因为当你想从「外包脏活」走到「并行干主线任务」时，这个选择会决定你掉不掉进协调地狱。

多 agent 协作有两种拓扑：

- **星型（汇聚）**：agent 之间不通信，每个只把状态单向写到一块板（或直接回主线），主线 / 人 / grader 负责汇总和 merge。代价是放弃横向实时协作，换来极简——没有并发写冲突、没有状态一致性问题、没有争用焦点。
- **白板（网状）**：每个 agent 都能往一块共享空间读写、彼此间接通信。强，但你要管并发写冲突、状态一致性、读写争用——这是 MiniMax Agent Team 那一级的重型设计，它自己也承认白板「会成为争用焦点、状态一致性难保证」。

> **大多数项目星型够用，白板是你确实需要 agent 边干边对齐时才上的重型方案。** L4 的脏活隔离是星型，把多个 feature 并行交给子 agent 跑、各写状态板、主线 merge——也是星型。只有当子任务强依赖、必须边干边对齐时，才轮到白板。

多 agent 不是免费的并行加速，是一笔需要用结构来偿还的负债（MiniMax 引 *Cost of Consensus*：无结构的「多」消耗 2.1–3.4× token 而准确率不升反降）。所以——**先想清楚怎么汇聚，再决定起几个 agent**。把这套落到模板，见 [并行编排层（可选）](parallel-agents.md)。
